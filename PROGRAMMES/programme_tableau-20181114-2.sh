			...
			then # URL OK
				...
				if [[ $ENCODAGE == "UTF-8" ]]
					then
						...
						# 3. contexte html
						perl /Users/nikahelicopter/Desktop/tal/PROJET-MOT-SUR-LE-WEB/minigrepmultilingue-v2/minigrepmultilingue-macosx "UTF-8" RU_Convention_UTF8.txt motif-regexp.txt
							...
							then
								...
								# LA SUITE : est-ce que ENCODAGEFILE vaut UTF8 ou non ??? à vous de jouer...
                                    ...
                                    # on refait le même traitement que ci-dessus !! en l'adaptant si nécessairethen
                                        ...
                                        nbmotif=$(egrep -coi "$3" ./DUMP-TEXT/$compteurtableau-$compteur.txt);
                                            # 3. contexte html
                                            perl ./minigrep/minigrepmultilingue.pl "utf-8" ./DUMP-TEXT/$compteurtableau-$compteur.txt ./minigrepmultilingue-macosx "UTF-8" RU_Convention_UTF8.txt motif-regexp.txt ;
                                                mv resultat-extraction.html ./CONTEXTES/$compteurtableau-$compteur.html ;
                                                # 4. index hierarchique
                                                egrep -o "\w+" ./DUMP-TEXT/$compteurtableau-$compteur.txt | sort | uniq -c | sort -r > ./DUMP-TEXT/index-$compteurtableau-$compteur.txt ;
                                                    # 5. bigramme
                                                ...
                                                cat bi3.txt | sort | uniq -c | sort -r >  ./DUMP-TEXT/bigramme-$compteurtableau-$compteur.txt ;
                                                #---------------------------------------------------------------------------
                                            echo "<tr><td>$compteur</td><td>$code_sortie</td><td><a target=\"_blank\" href=\"$ligne\">$ligne</a></td><td><a target=\"_blank\" href=\"../PAGES-ASPIREES/$compteurtableau-$compteur.html\">page aspirée n° $compteur</a></td><td>$ENCODAGEFILE</td><td><a target=\"_blank\" href=\"../DUMP-TEXT/$compteurtableau-$compteur.txt\">DUMP  n° $compteur</a></td><td><a href=\"../CONTEXTES/$compteurtableau-$compteur.txt\">CT $compteurtableau-$compteur</a></td><td><a href=\"../CONTEXTES/$compteurtableau-$compteur.html\">CTh $compteurtableau-$compteur</a></td><td>$nbmotif</td><td><a href=\"../DUMP-TEXT/index-$compteurtableau-$compteur.txt\">Ind $compteurtableau-$compteur</a></td><td><a href=\"../DUMP-TEXT/bigramme-$compteurtableau-$compteur.txt\">Bigr $compteurtableau-$compteur</a></td></tr>" >> $2 ;
									#else
								...
								if  [[ $reponse != "" ]]
									then
                                       ...
                                       ENCODAGEUCHARDET=$(uchardet ./PAGES-ASPIREES $compteurtableau-$compteur.html | cut -d"=" -f2);
                                        iconv -f $ENCODAGEUCHARDET -t utf-8 ./DUMP-TEXT/$compteurtableau-$compteur.txt  > ./DUMP-TEXT/$compteurtableau-$compteur-utf8.txt  ;
                                    if [[ $ENCODAGE == "UTF-8" ]]
                                    then
                                    echo -e "ENCODAGE initial <$ENCODAGE> OK : on passe au traitement \n";
                                    # aspiration de l'URL
